...
{
  GET_LIMIT_OFFSET;
  return sql_select(&posts_table_info, stmt_query_posts_by_community_id, community_id, offset, page_size);
}
...
void init_sql_manager()
{
  ...
  stmt_query_user_by_name = build_prepared_statement(sqlcon, "SELECT * FROM users WHERE name = BINARY ? AND NOT name = '[deleted]';");
